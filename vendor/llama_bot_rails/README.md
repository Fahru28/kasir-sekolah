...
# 2. Install the routes, chat interface, and migrations
...
LlamaBotRails uses the standard Rails engine migration pattern. Migrations are **not** auto-loaded; you must explicitly install them:

```bash
# Install migrations (copies them to your app's db/migrate/)
rails llama_bot_rails:install:migrations
...
Rails engines can either auto-load migrations or require explicit installation. We chose explicit installation because:

1. **Avoids duplicate table errors** - Auto-loading can cause `PG::DuplicateTable` errors when migrations exist in both the engine and the host app with different timestamps
2. **Better control** - You decide when to adopt new migrations from engine updates
3. **Standard Rails practice** - This is the [recommended approach](https://guides.rubyonrails.org/engines.html#migrations) in the Rails guides

### Updating migrations after gem updates

When you update llama_bot_rails, run the install command again to get any new migrations:
...
Rails will only copy migrations that don't already exist in your app.
