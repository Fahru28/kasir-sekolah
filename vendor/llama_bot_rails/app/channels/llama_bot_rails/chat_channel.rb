  ...
  class ChatChannel < ApplicationCable::Channel
    ...
    def receive(data)
      begin
        ...
        Rails.logger.debug "[LlamaBot] Got message from Javascript LlamaBot Frontend: #{data.inspect}"
      rescue => e
    ...
    def send_message_to_frontend(type, message, trace_info = nil)
      ...
      Rails.logger.debug "[LlamaBot] TRACE INFO DEBUG: Type: #{type}, Has trace info: #{trace_info.present?}"
    ...
    def listen_to_external_websocket(connection)
      begin
        while message = connection.read
          ...
          Rails.logger.debug "[LlamaBot] Received from external WebSocket: #{message_content}"
    ...
    def send_to_external_application(message)
      ...
      if @external_ws_connection
        begin
          ...
          Rails.logger.debug "[LlamaBot] Sent message to external WebSocket: #{payload}"
        rescue => e
