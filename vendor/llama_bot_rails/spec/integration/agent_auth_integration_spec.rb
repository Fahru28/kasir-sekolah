...
RSpec.describe 'Agent Authentication and Authorization Integration', type: :controller do
  ...
  describe 'Agent token authentication' do
    context 'with valid token' do
      ...
      it 'handles user resolver returning nil gracefully' do
        ...
        expect(error_response['error_code']).to eq('LLAMA_AUTH_006')
        expect(error_response['message']).to eq('User not found for the provided token')
      end
    ...
    context 'with valid token but no user_id' do
      ...
      it 'returns unauthorized when user resolver returns nil' do
        LlamaBotRails.user_resolver = ->(id) { nil }
    ...
    context 'with invalid token' do
      ...
      it 'returns unauthorized with detailed error for invalid tokens' do
        get :action_requiring_auth
        expect(response).to have_http_status(:unauthorized)

        error_response = JSON.parse(response.body)
        expect(error_response['error_code']).to eq('LLAMA_AUTH_004')
        expect(error_response['message']).to eq('Token signature verification failed')
        expect(error_response['details']['possible_causes']).to be_an(Array)
      end
    end
    ...
    context 'with expired token' do
      ...
      it 'returns unauthorized with detailed error for expired tokens' do
        get :action_requiring_auth
        expect(response).to have_http_status(:unauthorized)

        error_response = JSON.parse(response.body)
        # Expired tokens may show as invalid signature or expired depending on Rails version
        expect(['LLAMA_AUTH_004', 'LLAMA_AUTH_005']).to include(error_response['error_code'])
      end
    end
    ...
    context 'with wrong authentication scheme' do
      ...
      it 'returns unauthorized with detailed error about wrong scheme' do
        get :action_requiring_auth
        expect(response).to have_http_status(:unauthorized)

        error_response = JSON.parse(response.body)
        expect(error_response['error_code']).to eq('LLAMA_AUTH_002')
        expect(error_response['message']).to eq('Invalid authorization scheme')
        expect(error_response['details']['received_scheme']).to eq('Bearer')
        expect(error_response['details']['expected_scheme']).to eq('LlamaBot')
      end
    end

    context 'without authorization header' do
      it 'returns unauthorized with detailed error about missing header' do
        get :action_requiring_auth
        expect(response).to have_http_status(:unauthorized)

        error_response = JSON.parse(response.body)
        expect(error_response['error_code']).to eq('LLAMA_AUTH_001')
        expect(error_response['message']).to eq('Authorization header is missing')
      end
    end
  end
  ...
  describe 'security edge cases' do
    ...
    it 'prevents access with tokens for wrong user' do
      ...
      get :action_requiring_auth
      # Now returns unauthorized when user_resolver returns nil for wrong user
      expect(response).to have_http_status(:unauthorized)
